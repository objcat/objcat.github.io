...
`层级窗口`和`场景窗口`是息息相关的, 层级窗口中看到的内容一般情况下都可以在场景窗口显示出来, 比如我们新建的项目, 就可以在`Hierarchy`中看到一个`SampleScene`的场景对象, 里面有两个默认帮我们创建的`灯光和摄像机`

![](../camera/images/Pasted%20image%2020250813011523.png)

层级界面中直观的展现就是`Scene`场景界面, 我们可以在里面看到一个`摄像机`和一个`太阳`

![](images2/Pasted%20image%2020250813012106.png)

场景面板参照这个, 视频中截图下来的, 有调整光源, 切换2d和3d等功能

![](images2/Pasted%20image%2020250813012300.png)

## 🌲 摄像机

摄像机使用一句话概括就是, 我们游戏中屏幕能看到什么全靠摄像机来捕获, 它是我们视觉的基础

详细学习请点击下面的子文档

[跳转 camera](../camera/camera.md)
...
我们点击小方块, 可以看到显示出来了`三个轴`和`彩色小方块`, 他们都能点击并拖动来改变我们物体的位置, 你可以随意去拖动一下, 我们后面会详细讲
